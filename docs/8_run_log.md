...
Detailed, chronological record of every Kaggle GPU/CPU run this project
has executed: diagnostic results, ablation findings, promotions, bugs
found and fixed, and platform quirks discovered along the way.
[`4_next_steps.md`](./4_next_steps.md) stays short and decision-oriented;
this file is where the full evidence trail lives. Append new entries at
the end in chronological order -- do not edit past entries except to fix
factual errors (note the correction inline, don't silently rewrite
history).
